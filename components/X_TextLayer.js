...
const TextLayer = (props) => {
    ...
    useEffect(async () => {
        ...
        await setAdjustments(props.layer.adjustments)
    ...
    const layoutText = (props) => {
        ...
        _.each(textTag.current.children, (child, index) => {
            ...
            if (rows[currentRow].width + childWidth < layerWidth) {
                console.log('string fits on current row')
                ...
                console.log('string does not fit on current row')
                // It didn't fit so increment row and add new row to rows array
                currentRow++
                if (childIsSpace) {
                    console.log('string is a space')
                    ...
                    console.log('string is not a space')
